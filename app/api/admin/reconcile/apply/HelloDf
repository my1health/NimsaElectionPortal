...
async function repairSuccessfulTransaction(
  ...
  /*
   * If this payment does not exist,
   * recordPaidTransaction() can rebuild it
  ...
  const currentAmountKobo =
    Number(
      existingPayment.amount_kobo || 0
    );

  const currentVoteCount =
    Number(
      existingPayment.vote_count || 0
    );
  ...
  /*
   * Paystack's requested amount is the
   * authoritative site payment amount.
   */
  if (
    Number.isFinite(
      paystackAmountKobo
  ...
  /*
   * Successful Paystack transaction must
   * be successful in Supabase.
   */
  if (
    String(
...
export async function POST(
  ...
  try {
    ...
    /*
     * The browser body is intentionally NOT
     * trusted to decide what gets deleted.
     *
     * The server performs a fresh full
     * reconciliation.
     */
    await request
      .json()
    ...
    for (
      const transaction of successfulPaystack
    ) {
      try {
        const result =
    ...
    for (
      const payment of supabasePayments
    ) {
      /*
       * This payment exists in Paystack
       * successful transactions, so it is
       * legitimate and must remain.
       */
      if (
      ...
      try {
        /*
         * Delete the payment and all votes
         * belonging to that payment through
