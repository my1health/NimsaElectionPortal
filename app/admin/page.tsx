...
type Reconciliation = {
  summary: {
    ...
    paymentNotMarkedSuccess: number;
    supabaseOnly: number;

    totalPaystackAmountKobo: number;
    totalSupabaseAmountKobo: number;
    differenceKobo: number;

    paystackSuccessfulVotes: number;
    supabaseSuccessfulVotes: number;
  };
...
export default function AdminPage() {
  ...
  // ==========================================
  // PAYSTACK RECONCILIATION STATE
  // ==========================================

  const [reconciling, setReconciling] =
    useState(false);

  const [applyingCorrections, setApplyingCorrections] =
    useState(false);
  ...
  async function runReconciliation() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error ||
            "Unable to reconcile Paystack payments."
        );
      }
      ...
      setReconcileMessage(
        error?.message ||
          "Unable to check Paystack payments."
      );
    } finally {
  ...
  async function applyReconciliation() {
    ...
    const orphanPayments =
      reconciliation.summary.supabaseOnly;

    const repairCount =
      reconciliation.summary.missingPayment +
      reconciliation.summary.missingVotes +
      reconciliation.summary.paymentNotMarkedSuccess +
      reconciliation.summary.amountMismatch;

    const confirmed =
      window.confirm(
        `Apply full payment reconciliation?\n\n` +
          `Successful Paystack transactions: ${reconciliation.summary.paystackTransactions}\n` +
          `Payment/vote repairs: ${repairCount}\n` +
          `Supabase-only payments to remove: ${orphanPayments}\n\n` +
          `Any Supabase payment that is NOT found among successful Paystack transactions will be removed together with its associated vote rows.\n\n` +
          `Valid Paystack-backed votes will NOT be deleted automatically.\n\n` +
          `Continue?`
      );
    ...
    try {
       ...
       * IMPORTANT:
       *
       * We deliberately do NOT send payment references
       * from the browser.
       *
       * The server independently fetches Paystack and
       * determines which Supabase records are safe to
       * repair or delete.
       */
      const response = await fetch(
        "/api/admin/reconcile/apply",
        {
          method: "POST",
          headers: {
            "Content-Type":
      ...
      if (!response.ok) {
        throw new Error(
          data.error ||
            "Unable to apply reconciliation."
        );
      }
       ...
       *
       * This is done after the database changes
       * have completed.
       */
      ...
      setReconcileMessage(
        `Reconciliation completed. ` +
          ...
          `votes added: ${result.votesAdded || 0}, ` +
          `orphan payments removed: ${result.orphanPaymentsDeleted || 0}, ` +
          `orphan votes removed: ${result.orphanVotesDeleted || 0}, ` +
        ...
        >
          ...
          >
            <div>
              ...
              >
                Compare successful Paystack
                transactions against Supabase
                payments and recorded votes.
              </p>
            </div>
          ...
          {!reconciliation ? (
            ...
            >
              ...
              >
                Click “Check Paystack” to compare
                Paystack transactions with your
                voting database.
              </span>
            </div>
          ) : (
            <>
              ...
              >
                ...
                <ReconCard
                  ...
                  subtitle="Paystack payments found"
                  good
                />

                <ReconCard
                  title="PAYSTACK VOTES"
                  value={
                    reconciliation.summary
                      .paystackSuccessfulVotes
                  }
                  subtitle="Expected successful votes"
                  good
                />

                <ReconCard
                  title="SUPABASE VOTES"
                  value={
                    reconciliation.summary
                      .supabaseSuccessfulVotes
                  }
                  subtitle="Currently recorded"
                />
                ...
                >
                  ...
                  {reconciliation.summary
                    ...
                    <>
                      ...
                      transaction(s) with more
                      vote rows than expected; these
                      votes are deliberately not
                  ...
                  {reconciliation.summary
                    ...
                    <>
                      ...
                      transaction(s) without enough
                      metadata for completely automatic
                      recovery.
                    </>
                  )}
                </div>
