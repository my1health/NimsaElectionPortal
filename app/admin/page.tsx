...
export default function AdminPage() {
  ...
  async function applyReconciliation() {
    ...
    if (!safeItems.length) {
      setReconcileMessage(
        "There are no safe corrections to apply."
      );
    ...
    const confirmed = window.confirm(
      `Paystack reconciliation found ${references.length} transaction(s) that can be safely corrected.\n\n` +
        `This will NOT automatically delete extra votes or change amount mismatches.\n\n` +
            ...
            <>
              ...
              {(reconciliation.summary
                ...
                >
                  ...
                  Amount mismatches and extra
                  votes are deliberately NOT
                  automatically changed. We need
                  to inspect the affected
                  transactions before deleting or
                  altering votes.
                </div>
              )}
                      ...
                      reconciliation.items.map(
                        (item) => (
                          ...
                          >
                            ...
                            >
                              {typeof item.paystackAmountKobo ===
                              "number"
                                ? `₦${(
                                    item.paystackAmountKobo /
                                    100
                                  ).toLocaleString()}`
                                : "—"}
                            </td>
                            ...
                            >
                              {typeof item.supabaseAmountKobo ===
                              "number"
                                ? `₦${(
                                    item.supabaseAmountKobo /
                                    100
                                  ).toLocaleString()}`
                                : "—"}
                            </td>
                            ...
                            >
                              {typeof item.paystackVoteCount ===
                              "number"
                                ? item.paystackVoteCount
                                : "—"}
                            </td>
                            ...
                            >
                              {typeof item.supabaseVoteCount ===
                              "number"
                                ? item.supabaseVoteCount
                                : "—"}
                            </td>
